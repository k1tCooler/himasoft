...
class MySql(SqlBase):
    ...
    def get_proxy_with_id(self, table_name, id):
        proxy = Proxy()
        try:
            command = "SELECT * FROM {0} WHERE id=\'{1}\'".format(
                table_name, id)
    ...
    def get_proxy_random(self, table_name):
        proxy = None
        # 获取随机记录
        try:
            command = '''SELECT * FROM {0} WHERE id >= (
                        (SELECT 
                          MAX(id) 
                        FROM
                          {0}) - 
            ...
            logging.exception('mysql get_proxy_random exception msg:%s' % e)
        return proxy
    ...
    def insert_data(self, command, data, commit=False):
        try:
            logging.debug('mysql insert_data command:%s, data:%s' %
                          (command, data))
